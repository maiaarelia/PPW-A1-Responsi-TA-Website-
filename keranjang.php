...
if (isset($_POST['add_to_cart'])) {
    ...
    $query = mysqli_query($conn, "INSERT INTO keranjang (id_produk, USER_ID) VALUES ('$id_produk', '$user_id')");
    if ($query) {
        echo "
            <script>
            ...
            if (isset($_SESSION['user'])) {
                ...
                if ($jml > 0) {
                    if (isset($_SESSION['kd_cs'])) {
                        ...
                        $result = mysqli_query($conn, "SELECT k.id_keranjang as keranjang, k.kode_produk as kd, p.NAMA_PRODUK as nama, k.qty as jml, p.GAMBAR_PRODUK as gambar, p.HARGA_PRODUK as hrg FROM keranjang k join produk_kecantikan p on k.kode_produk=p.ID_PRODUK WHERE kode_customer = '$kode_cs'");
                        $no = 1;
                        $hasil = 0;
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                                ...
                                <tr>
                                    <th scope="row"><?= $no;  ?></th>
                                    <td><img src="img/facewash/<?= $row['gambar']; ?>" width="100"></td>
                                    <td><?= $row['nama']; ?></td>
